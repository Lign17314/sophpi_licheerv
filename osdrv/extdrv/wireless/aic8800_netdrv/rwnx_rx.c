...
// Timeout to send keep alive msg.
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,14,0)
void keep_alive_timer_handler(unsigned long data)
#else
void keep_alive_timer_handler(struct timer_list *t)
#endif
{
    if (!g_custom_msg_vnet.keep_alive.first_time_check == true) {
        g_custom_msg_vnet.keep_alive.first_time_check = false;
        g_custom_msg_vnet.keep_alive.local_count = g_custom_msg_vnet.keep_alive.alive_count;
        schedule_work(&g_custom_msg_vnet.keep_alive.work_queue);
        mod_timer(&g_custom_msg_vnet.keep_alive.timer, jiffies + msecs_to_jiffies(g_custom_msg_vnet.vnet_setting_cfm.keep_alive));

    } else {
        if (LOCAL_ABS(g_custom_msg_vnet.keep_alive.local_count - g_custom_msg_vnet.keep_alive.alive_count) > 6) {
        //if (g_custom_msg_vnet.keep_alive.local_count + 6 < g_custom_msg_vnet.keep_alive.alive_count) {
            printk("AIC8800x can`t keep alive!\n");
            g_custom_msg_vnet.keep_alive.status = KEEP_ALIVE_BREAK;
        } else {
            schedule_work(&g_custom_msg_vnet.keep_alive.work_queue);
            g_custom_msg_vnet.keep_alive.local_count += 1;
            mod_timer(&g_custom_msg_vnet.keep_alive.timer, jiffies + msecs_to_jiffies(g_custom_msg_vnet.vnet_setting_cfm.keep_alive));
        }
    }
}
...
void rwnx_rx_handle_msg(void *dev, struct ipc_e2a_msg *msg)
{
    ...
    switch(msg->id) {
        case CUST_CMD_CONNECT_CFM:
            ...
            sprintf(sync_buff, "MSG: %x, start connect\r\n", CUST_CMD_CONNECT_CFM);
        ...
        case CUST_CMD_CONNECT_IND:
            ...
            printk("ip: %d.%d.%d.%d,  gw: %d.%d.%d.%d,  mk: %d.%d.%d.%d\n",
                ...
                (g_custom_msg_vnet.connect_ind.mk >> 0 ) & 0xff,   (g_custom_msg_vnet.connect_ind.mk >> 8 ) & 0xff,
                (g_custom_msg_vnet.connect_ind.mk >> 16) & 0xff,   (g_custom_msg_vnet.connect_ind.mk >> 24) & 0xff);

            printk("VNET_DEV: Allow send data-pkg\n");
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, ssid: %s, rssi: %d, ip: %08x, gw: %08x, mk: %08x\r\n", CUST_CMD_CONNECT_IND, g_custom_msg_vnet.connect_ind.ussid,
                    g_custom_msg_vnet.connect_ind.rssi, g_custom_msg_vnet.connect_ind.ip, g_custom_msg_vnet.connect_ind.gw, g_custom_msg_vnet.connect_ind.mk);
            rwnx_aic_cdev_fill_mem(sync_buff);
            #endif
            break;
        case CUST_CMD_DISCONNECT_CFM:
            ...
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, start disconnect\r\n", CUST_CMD_DISCONNECT_CFM);
            rwnx_aic_cdev_fill_mem(sync_buff);
            #endif
            break;
        case CUST_CMD_DISCONNECT_IND:
            ...
            #endif
            break;
        case CUST_CMD_OPEN_BLE_CFM:
            printk("MCU-BLE-STATE: open ble task\r\n");
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, open ble task\r\n", CUST_CMD_OPEN_BLE_CFM);
            rwnx_aic_cdev_fill_mem(sync_buff);
            #endif
            break;
        case CUST_CMD_OPEN_BLE_IND:
            printk("MCU-BLE-STATE: ble task init\r\n");
            g_custom_msg_vnet.ble_status = BLE_STATUS_INIT;
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, ble task init\r\n", CUST_CMD_OPEN_BLE_IND);
            rwnx_aic_cdev_fill_mem(sync_buff);
            #endif
            break;
        case CUST_CMD_CLOSE_BLE_CFM:
            printk("MCU-BLE-STATE: close ble task\r\n");
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, close ble task\r\n", CUST_CMD_CLOSE_BLE_CFM);
            rwnx_aic_cdev_fill_mem(sync_buff);
            #endif
            break;
        case CUST_CMD_CLOSE_BLE_IND:
            printk("MCU-BLE-STATE: ble task delete\r\n");
            g_custom_msg_vnet.ble_status = BLE_STATUS_DEINIT;
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, ble task delete\r\n", CUST_CMD_CLOSE_BLE_IND);
            rwnx_aic_cdev_fill_mem(sync_buff);
            #endif
            break;
        case CUST_CMD_ENTER_SLEEP_CFM:
            ...
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, enter sleep\r\n", CUST_CMD_ENTER_SLEEP_CFM);
            rwnx_aic_cdev_fill_mem(sync_buff);
            #endif
            break;
        ...
        case CUST_CMD_GET_MAC_ADDR_CFM:
            ...
            #ifdef CONFIG_APP_FASYNC
            if (g_custom_msg_vnet.comp_sign_get_mac_ready == true) {
                sprintf(sync_buff, "MSG: %x, mac: %x.%x.%x.%x.%x.%x\r\n", CUST_CMD_GET_MAC_ADDR_CFM,
                    g_custom_msg_vnet.macaddr_cfm.mac_addr[0], g_custom_msg_vnet.macaddr_cfm.mac_addr[1],
                    g_custom_msg_vnet.macaddr_cfm.mac_addr[2], g_custom_msg_vnet.macaddr_cfm.mac_addr[3],
                    g_custom_msg_vnet.macaddr_cfm.mac_addr[4], g_custom_msg_vnet.macaddr_cfm.mac_addr[5]);
                rwnx_aic_cdev_fill_mem(sync_buff);
        ...
        case CUST_CMD_GET_WLAN_STATUS_CFM:
            ...
            if(g_custom_msg_vnet.get_wlan_cfm.status == WLAN_DISCONNECT) {
                ...
                printk("ip: %d.%d.%d.%d,  gw: %d.%d.%d.%d, mk: %d.%d.%d.%d\n",
                    ...
                    (g_custom_msg_vnet.get_wlan_cfm.mk >> 0 ) & 0xff,   (g_custom_msg_vnet.get_wlan_cfm.mk >> 8 ) & 0xff,
                    (g_custom_msg_vnet.get_wlan_cfm.mk >> 16) & 0xff,   (g_custom_msg_vnet.get_wlan_cfm.mk >> 24) & 0xff);
                g_custom_msg_vnet.wlan_status = WLAN_CONNECTED;
                if (vnet_dev) {
                    netif_tx_wake_all_queues(vnet_dev);
                    printk("VNET_DEV: Allow send data-pkg\n");
            ...
            break;
        case CUST_CMD_GET_VNET_SETTING_CFM:
            memcpy(&g_custom_msg_vnet.vnet_setting_cfm, msg->param, sizeof(struct custom_msg_aic8800x_vnet_setting_cfm));
            printk("<vnet-mode>\n"
                   " - keep_alive:   %d\n"
                   " - ble_enable:   %d\n"
                   " - lzma_enable:  %d\n"
                   ...
                   " - aicplatform:  %d\n"
                   " - upgrade_addr: %08x\n",
                   g_custom_msg_vnet.vnet_setting_cfm.keep_alive,  g_custom_msg_vnet.vnet_setting_cfm.ble_enable,
                   g_custom_msg_vnet.vnet_setting_cfm.lzma_enable, g_custom_msg_vnet.vnet_setting_cfm.filter_mode,
                   g_custom_msg_vnet.vnet_setting_cfm.aicplatform, g_custom_msg_vnet.vnet_setting_cfm.upgrade_addr);
            // If enable keep-alive, start timer
            if (g_custom_msg_vnet.vnet_setting_cfm.keep_alive) {
                g_custom_msg_vnet.keep_alive.local_count = 0;
                g_custom_msg_vnet.keep_alive.alive_count = 0;
                g_custom_msg_vnet.keep_alive.first_time_check = true;
                g_custom_msg_vnet.keep_alive.status = KEEP_ALIVE_ENABLE;
                INIT_WORK(&g_custom_msg_vnet.keep_alive.work_queue, keep_alive_timer_work);

                #if LINUX_VERSION_CODE < KERNEL_VERSION(4,14,0)
                init_timer(&g_custom_msg_vnet.keep_alive.timer);
                g_custom_msg_vnet.keep_alive.timer.expires = jiffies + msecs_to_jiffies(g_custom_msg_vnet.vnet_setting_cfm.keep_alive);
                g_custom_msg_vnet.keep_alive.timer.function = keep_alive_timer_handler;
                #else
                g_custom_msg_vnet.keep_alive.timer.expires = jiffies + msecs_to_jiffies(g_custom_msg_vnet.vnet_setting_cfm.keep_alive);
                timer_setup(&g_custom_msg_vnet.keep_alive.timer, keep_alive_timer_handler, 0);
                #endif
                add_timer(&g_custom_msg_vnet.keep_alive.timer);
            }
            break;
        case CUST_CMD_START_AP_CFM:
        case CUST_CMD_START_P2PGO_CFM:
            printk("MCU-AP-STATE: start AP\n");
            break;
        case CUST_CMD_START_AP_IND:
        case CUST_CMD_START_P2PGO_IND:
            memcpy(&g_custom_msg_vnet.ap_status_cfm, msg->param, sizeof(struct custom_msg_ap_status_cfm));
            if(g_custom_msg_vnet.ap_status_cfm.status == AIC_AP_START) {
                printk("MCU-AP-STATE: start AP success, ip: %d.%d.%d.%d,  gw: %d.%d.%d.%d, mk: %d.%d.%d.%d\n",
                    (g_custom_msg_vnet.ap_status_cfm.ip >> 0 ) & 0xff,   (g_custom_msg_vnet.ap_status_cfm.ip >> 8 ) & 0xff,
                    (g_custom_msg_vnet.ap_status_cfm.ip >> 16) & 0xff,   (g_custom_msg_vnet.ap_status_cfm.ip >> 24) & 0xff,
                    (g_custom_msg_vnet.ap_status_cfm.gw >> 0 ) & 0xff,   (g_custom_msg_vnet.ap_status_cfm.gw >> 8 ) & 0xff,
                    (g_custom_msg_vnet.ap_status_cfm.gw >> 16) & 0xff,   (g_custom_msg_vnet.ap_status_cfm.gw >> 24) & 0xff,
                    (g_custom_msg_vnet.ap_status_cfm.mk >> 0 ) & 0xff,   (g_custom_msg_vnet.ap_status_cfm.mk >> 8 ) & 0xff,
                    (g_custom_msg_vnet.ap_status_cfm.mk >> 16) & 0xff,   (g_custom_msg_vnet.ap_status_cfm.mk >> 24) & 0xff);
                g_custom_msg_vnet.aic_mode_status = AIC_MODE_AP_CONFIG;
                #ifdef CONFIG_APP_FASYNC
                sprintf(sync_buff, "MSG: %x, start AP success, ip: %08x, gw: %08x, mk: %08x\r\n", CUST_CMD_START_AP_IND,
                ...
                #ifdef CONFIG_APP_FASYNC
                sprintf(sync_buff, "MSG: %x, start AP fail\r\n", CUST_CMD_START_AP_IND);
                rwnx_aic_cdev_fill_mem(sync_buff);
                #endif
            } else {
                // other
            ...
            if (g_custom_msg_vnet.vnet_setting_cfm.filter_mode == VNET_FILTER_SHARED)
                netif_tx_wake_all_queues(vnet_dev);
            break;
        case CUST_CMD_ASSOC_AP_IND:
            ...
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, AP-ASSOC: %x.%x.%x.%x.%x.%x\r\n", CUST_CMD_ASSOC_AP_IND,
                    g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[0], g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[1],
                    g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[2], g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[3],
                    g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[4], g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[5]);
            rwnx_aic_cdev_fill_mem(sync_buff);
        ...
        case CUST_CMD_DISASSOC_AP_IND:
            ...
            printk("%x.%x.%x.%x.%x.%x\n",
                    ...
                    g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[4], g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[5]);
            #ifdef CONFIG_APP_FASYNC
            sprintf(sync_buff, "MSG: %x, AP-DISASSOC: %x.%x.%x.%x.%x.%x\r\n", CUST_CMD_DISASSOC_AP_IND,
                    g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[0], g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[1],
                    g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[2], g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[3],
                    g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[4], g_custom_msg_vnet.ap_assoc_sta_addr_ind.sub_sta_addr[5]);
            rwnx_aic_cdev_fill_mem(sync_buff);
        ...
        case CUST_CMD_CHANGE_AP_MODE_CFM:
            ...
            if(g_custom_msg_vnet.aic_mode_status == AIC_MODE_AP_CONFIG) {
                ...
                #ifdef CONFIG_APP_FASYNC
                sprintf(sync_buff, "MSG: %x, AIC_MODE_AP_CONFIG\r\n", CUST_CMD_CHANGE_AP_MODE_CFM);
                rwnx_aic_cdev_fill_mem(sync_buff);
                #endif
                ...
                #ifdef CONFIG_APP_FASYNC
                sprintf(sync_buff, "MSG: %x, AIC_MODE_AP_DIRECT\r\n", CUST_CMD_CHANGE_AP_MODE_CFM);
                rwnx_aic_cdev_fill_mem(sync_buff);
                #endif
            } else {
                // other
        ...
        case CUST_CMD_SCAN_WIFI_CFM:
            printk("MCU-SCAN-WIFI: start\n");
            break;
        case CUST_CMD_SCAN_WIFI_IND:
            printk("MCU-SCAN-WIFI:\n");
            g_custom_msg_vnet.scan_pkt_recv_num++;
            memcpy(&g_custom_msg_vnet.scan_wifi_cfm, msg->param, msg->param_len);
            printk("scan_num=%d\n", g_custom_msg_vnet.scan_wifi_cfm.scan_num);
            g_custom_msg_vnet.scan_wifi_cfm_ptr = g_custom_msg_vnet.scan_wifi_cfm.cfm;
            if (g_custom_msg_vnet.scan_pkt_recv_num > 1) {
                loop_idx = (g_custom_msg_vnet.scan_pkt_recv_num - 1) * 32;
            }
            do {
                loop_idx++;
                printk("%2d:(%3d dBm) CH=%3d AKM=%3d BSSID=%02x:%02x:%02x:%02x:%02x:%02x SSID=%s\n",
                    loop_idx, g_custom_msg_vnet.scan_wifi_cfm_ptr->rssi,
                ...
                if ((loop_idx % 32) == 0)
                    break;
            } while(loop_idx < g_custom_msg_vnet.scan_wifi_cfm.scan_num);
            break;
        case CUST_CMD_WIFI_FAIL_IND:
            memcpy(&g_custom_msg_vnet.wifi_proc_fail, msg->param, msg->param_len);
            switch (g_custom_msg_vnet.wifi_proc_fail) {
                case PROCESS_START_STA:
                    printk("start sta fail\n");
                    break;
                case PROCESS_STOP_STA:
                    printk("stop sta fail\n");
                    break;
                case PROCESS_START_AP:
                    printk("start ap fail\n");
                    break;
                case PROCESS_STOP_AP:
                    printk("start ap fail\n");
                    break;
                case PROCESS_SCANNING:
                    printk("start scan fail\n");
                    break;
                default:
                    printk("wifi_proc type err\n");
            }
            break;
        case CUST_CMD_HOST_OTA_CFM:
            ...
            if(g_custom_msg_vnet.ota_status == OTA_STEP_FLASH_ERASE_OK) {
                ...
                aicwf_mcu_ota_confirm_ready();
                #ifdef CONFIG_APP_FASYNC
                sprintf(sync_buff, "MSG: %x, host-OTA success\r\n", CUST_CMD_HOST_OTA_CFM);
                rwnx_aic_cdev_fill_mem(sync_buff);
                #endif
            } else if(g_custom_msg_vnet.ota_status == OTA_STEP_LZMA_CRC_CHECK_OK) {
                printk("MCU-OTA: finish LZMA-OTA...\n");
                aicwf_mcu_ota_confirm_ready();
                #ifdef CONFIG_APP_FASYNC
                sprintf(sync_buff, "MSG: %x, LZMA host-OTA success\r\n", CUST_CMD_HOST_OTA_CFM);
                rwnx_aic_cdev_fill_mem(sync_buff);
                #endif
            } else {
                printk("MCU-OTA: OTA error: %d. Please restart OTA.\n", g_custom_msg_vnet.ota_status);
                #ifdef CONFIG_APP_FASYNC
                sprintf(sync_buff, "MSG: %x, host-OTA fail\r\n", CUST_CMD_HOST_OTA_CFM);
                rwnx_aic_cdev_fill_mem(sync_buff);
                #endif
            }
            break;
        case CUST_CMD_HOST_KEEP_ALIVE_CFM:
            g_custom_msg_vnet.keep_alive.alive_count += 1;
            //printk("host_keep_alive: %d\n", g_custom_msg_vnet.keep_alive_count);
            break;
        case CUST_CMD_HOST_SET_WLAN_IP_CFM:
            printk("MCU-WLAN: set ip success\n");
            break;
        case CUST_CMD_START_RFTEST_CFM:
            printk("MCU-WLAN: start rftest\n");
            break;
        #ifdef CONFIG_HOSTBT
        case CUST_CMD_HOSTBT_CHANNEL_CFM:
            printk("HOSTBT:send cfm\n");
            break;
        case CUST_CMD_HOSTBT_CHANNEL_IND:
            printk("HOSTBT: get msg ind\n");
            g_custom_msg_vnet.hostbt_channel_ind = (struct custom_msg_hostbt_channel_ind *)&msg->param;
            bt_sdio_recv(g_custom_msg_vnet.hostbt_channel_ind->bt_data, g_custom_msg_vnet.hostbt_channel_ind->data_len);
            break;
        case CUST_CMD_START_HOSTBT_CFM:
            printk("HOSTBT: start cfm\n");
            break;
        case CUST_CMD_STOP_HOSTBT_CFM:
            printk("HOSTBT: stop cfm\n");
            break;
        #endif

        default:
            printk("%s msg %x invaild\n", __func__, msg->id);
    }
    if (msg->id == g_cmd_mgr.cfm_id) {
        // set cmd-cfm status, and then return msg to application.
        rwnx_tx_msg_cfm_ready();
    }
}
#endif
