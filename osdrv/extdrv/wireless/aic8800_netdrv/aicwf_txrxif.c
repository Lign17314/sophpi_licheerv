        ...
        while(aicwf_next_ptk(skb)) {
            ...
            if((skb->data[2] & SDIO_TYPE_CFG) != SDIO_TYPE_CFG) { // type : data
                ...
                if((*(msg + 2) & 0x7f) == SDIO_TYPE_CFG_CMD_RSP) {
                    ...
                    if (rftest_enable)
                    {
                        rwnx_rx_handle_rftest_msg(rx_priv->sdiodev->rwnx_hw, (struct ipc_e2a_msg *)(msg + 4));
                    }
                    else
                    #endif
                    {
    ...
    while (1) {
        ...
        if((skb->data[2] & USB_TYPE_CFG) != USB_TYPE_CFG) { // type : data
            ...
            if((*(msg + 2) & 0x7f) == USB_TYPE_CFG_CMD_RSP) {
                ...
                if (rftest_enable)
                {
                    rwnx_rx_handle_rftest_msg(rx_priv->usbdev->rwnx_hw, (struct ipc_e2a_msg *)(msg + 4));
                }
                else
                #endif
                {
...
int aicwf_process_msg_rxframes(struct aicwf_rx_priv *rx_priv)
{
    ...
    while (1) {
        ...
        if((skb->data[2] & USB_TYPE_CFG) != USB_TYPE_CFG) { // type : data
            ...
            rwnx_rx_handle_data(rx_priv, skb);
