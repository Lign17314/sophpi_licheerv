...
static struct dbg_rftest_cmd_cfm cfm = {{0,}};
...
static int aic_priv_cmd_set_tx (struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
	...
	if (argc > 7) {
		if (dev->chipid == PRODUCT_ID_AIC8800){
...
static int aic_priv_cmd_get_mac_addr (struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
	...
	if (dev->chipid == PRODUCT_ID_AIC8800MC) {
...
static int aic_priv_cmd_rdwr_pwrlvl (struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
	...
	if(dev->chipid == PRODUCT_ID_AIC8800M40 || dev->chipid == PRODUCT_ID_AIC8800M80X2){
...
static int aic_priv_cmd_rdwr_pwrofst (struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
	...
	if (func == 0) { // read cur
		...
		if ((argc > 4) && (dev->chipid == PRODUCT_ID_AIC8800M40 || dev->chipid == PRODUCT_ID_AIC8800M80X2)) {
			u8_l type = (u8_l)command_strtoul(argv[2], NULL, 16);
		...
		} else if ((argc > 3) && (dev->chipid != PRODUCT_ID_AIC8800M40) && (dev->chipid != PRODUCT_ID_AIC8800M80X2)) {
	...
	if (dev->chipid == PRODUCT_ID_AIC8800MC) { // 3 = 3 (2.4g)
		res_len = 3;
	} else if (dev->chipid == PRODUCT_ID_AIC8800M40) { // 3 * 2 (2.4g) + 3 * 6 (5g)
		res_len = 3 * 3 + 3 * 6;
	} else if (dev->chipid == PRODUCT_ID_AIC8800M80X2) { // ant0/1
		res_len = ( 3 * 3 + 3 * 6 ) * 2;
	} else {
		res_len = 3 + 4;
	}
...
static int aic_priv_cmd_rdwr_efuse_pwrofst (struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
	...
	if (dev->chipid == PRODUCT_ID_AIC8800MC) { // 6 = 3 (2.4g) * 2
		res_len = 3 * 2;
	} else if (dev->chipid == PRODUCT_ID_AIC8800M40) { // 3 * 2 (2.4g) + 3 * 6 (5g)
		res_len = (3 * 3 + 3 * 6) * 2;
	} else if(dev->chipid == PRODUCT_ID_AIC8800M80X2) { // 3 * 2 (2.4g) *2 + 3 * 6 (5g) *2
		res_len = (3 * 3 + 3 * 6) * 2 * 2;
	} else { // 7 = 3(2.4g) + 4(5g)
		res_len = 3 + 4;
	}
...
static int aic_priv_cmd_set_txpwr_loss(struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
	...
	if (argc > 1) {
		...
		if (dev->chipid == PRODUCT_ID_AIC8800M40){
			set_txpwr_loss_ofst(func);
			rwnx_send_txpwr_lvl_v3_req(g_rwnx_hw);
		}else if(dev->chipid == PRODUCT_ID_AIC8800M80X2){
			set_txpwr_loss_ofst(func);
			rwnx_send_txpwr_lvl_v4_req(g_rwnx_hw);
		}else if(dev->chipid == PRODUCT_ID_AIC8800MC){
			set_txpwr_loss_ofst(func);
			rwnx_send_txpwr_lvl_req(g_rwnx_hw);
		}else{
...
static int aic_priv_cmd_mcu_reboot (struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
    u8_l func = 0;

    if (argc > 1) {
        func = (s8_l)command_strtoul(argv[1], NULL, 10);
        if (func == PRODUCT_ID_AIC8800) {
            AICWFDBG(LOGINFO, "AIC8800 reboot\n");
            // todo
        } else if (func == PRODUCT_ID_AIC8800MC) {
            AICWFDBG(LOGINFO, "AIC8800MC reboot\n");
            // todo
        } else if (func == PRODUCT_ID_AIC8800M40) {
            AICWFDBG(LOGINFO, "AIC8800M40 reboot\n");
            rwnx_send_dbg_mem_mask_write_req(rwnx_hw, 0x7000000C, 0x00030000, 0x01030000);
            rwnx_send_dbg_mem_mask_write_req(rwnx_hw, 0x70000000, 0x00000001, 0x00000001);
            rwnx_send_dbg_mem_write_req(rwnx_hw, 0x70001400, 0x0000000F);
            rwnx_send_dbg_mem_mask_write_req(rwnx_hw, 0x70001408, 0x00000002, 0x00000002);
            //rwnx_send_dbg_mem_write_req(rwnx_hw, 0x70000028, 0x00000000);
        } else if (func == PRODUCT_ID_AIC8800M80X2) {
            AICWFDBG(LOGINFO, "AIC8800M80X2 reboot\n");
            // todo
        } else {
...
static int aic_priv_cmd_mcu_gpio (struct rwnx_hw *rwnx_hw, int argc, char *argv[], char *command)
{
    int gpio_type = 0;
    int gpio_idx = 0;
    int gpio_dir = 0;
    ...
    if (argc < 4) {
        AICWFDBG(LOGERROR, "wrong args %d\n", __LINE__);
        return -EINVAL;
    }

    gpio_type = (int)command_strtoul(argv[1], NULL, 10);
    gpio_idx  = (int)command_strtoul(argv[2], NULL, 10);
    gpio_dir  = (int)command_strtoul(argv[3], NULL, 10);

    if (gpio_dir == GPIODIR_OUTPUT) {
        if (argc < 5) {
            AICWFDBG(LOGERROR, "wrong args %d\n", __LINE__);
            return -EINVAL;
        }
    }
    gpio_lvl  = (int)command_strtoul(argv[4], NULL, 10);

    if (gpio_type == GPIO_TYPEA) {
        if (gpio_dir == GPIODIR_OUTPUT) {
            gpioa_init(rwnx_hw, gpio_idx);
            gpioa_dir_out(rwnx_hw, gpio_idx);
            if (gpio_lvl == GPIOVAL_LOW) {
                gpioa_clr(rwnx_hw, gpio_idx);
            } else if (gpio_lvl == GPIOVAL_HIGH) {
                gpioa_set(rwnx_hw, gpio_idx);
            } else {
                AICWFDBG(LOGERROR, "wrong args %d\n", __LINE__);
                return -EINVAL;
            }
    ...
    } else if(gpio_type == GPIO_TYPEB) {
        if (gpio_dir == GPIODIR_OUTPUT) {
            gpiob_init(rwnx_hw, gpio_idx);
            gpiob_dir_out(rwnx_hw, gpio_idx);
            if (gpio_lvl == GPIOVAL_LOW) {
                gpiob_clr(rwnx_hw, gpio_idx);
            } else if (gpio_lvl == GPIOVAL_HIGH) {
                gpiob_set(rwnx_hw, gpio_idx);
            } else {
                AICWFDBG(LOGERROR, "wrong args %d\n", __LINE__);
                return -EINVAL;
            }
...
//Reserve for new aic_priv_cmd.
    {"mcu_reboot", aic_priv_cmd_mcu_reboot, "mcu reboot"},
    {"mcu_gpio", aic_priv_cmd_mcu_gpio,
      "<gpio_type> <gpio_idx> <gpio_dir> <gpio_lvl> = set gpio"},
...
int aicwf_rftest_handler(char *command)
{
	...
	int argc;
	struct rwnx_hw *p_rwnx_hw = g_rwnx_hw;
