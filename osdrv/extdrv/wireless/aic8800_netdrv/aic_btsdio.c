...
static struct custom_msg_hostbt_channel_req bt_test_send_req;
        ...
        loff_t *pos_p)
{
    ...
    bt_test_send_req.hdr.cmd_id = CUST_CMD_HOSTBT_CHANNEL_REQ;
    bt_test_send_req.data_len = skb->len;
    memcpy(bt_test_send_req.bt_data, skb->data, skb->len);
    err = rwnx_tx_msg((u8 *)&bt_test_send_req, sizeof(bt_test_send_req), &cmd_cfm, cmd_result);
