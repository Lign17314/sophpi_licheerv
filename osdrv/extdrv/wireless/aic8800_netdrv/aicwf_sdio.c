...
#ifdef CONFIG_SDIO_IPC_FC
#define rSDIO_FN0UNT_F0     0xF0
#define rSDIO_FN1UNT_08     0x08
#define rSDIO_FN0UNT_F0_BIT 0x04
#define rSDIO_FN1UNT_08_BIT 0x10
...
int aicwf_sdio_flow_ctrl(struct aic_sdio_dev *sdiodev)
{
    ...
    while (true) {
        ...
        #ifdef CONFIG_SDIO_IPC_FC
        if (g_custom_msg_vnet.vnet_setting_cfm.aicplatform == AIC8800MC) {
            sdio_claim_host(sdiodev->func);
            ipc_fc_reg = sdio_f0_readb(sdiodev->func, rSDIO_FN0UNT_F0, &ret);
            sdio_release_host(sdiodev->func);
            if (ret) {
                sdio_err("f0 read err\n");
                return -1;
            }
            ipc_fc = ipc_fc_reg & rSDIO_FN0UNT_F0_BIT;
        } else if (g_custom_msg_vnet.vnet_setting_cfm.aicplatform == AIC8800M40) {
            sdio_claim_host(sdiodev->func);
            ipc_fc_reg = sdio_readb(sdiodev->func, rSDIO_FN1UNT_08, &ret);
            sdio_release_host(sdiodev->func);
            if (ret) {
                sdio_err("f1 read err\n");
                return -1;
            }
            ipc_fc = ipc_fc_reg & rSDIO_FN1UNT_08_BIT;
        } else {
            sdio_err("aic platform not support ipc fc\n");
            return -1;
        }

        if (ipc_fc != 0) {
            if (ipc_count >= FLOW_CTRL_RETRY_COUNT) {
                ret = -fc_reg;
                sdio_err("ipc fc:%d, %d\n", ipc_fc, fc_reg);
                ret = -2;
                  break;
              ...
              if (ipc_count < 30)
                  udelay(200);
              else if(ipc_count < 40)
                  msleep(2);
              else
                  msleep(10);
              continue;
        }
        #endif /* CONFIG_SDIO_IPC_FC */
...
static void aicwf_sdio_bus_stop(struct device *dev)
{
    ...
    #ifdef CONFIG_IRQRX_THREAD
    if (sdiodev->irqrx_thread) {
        complete(&sdiodev->irqrx_trgg);
        kthread_stop(sdiodev->irqrx_thread);
        sdiodev->irqrx_thread = NULL;
    }
    #endif
...
int bustx_thread_prio = 0;
int busrx_thread_prio = 0;
...
int sdio_bustx_thread(void *data)
{
    ...
    if (bustx_thread_prio > 0) {
        ...
        param.sched_priority = (bustx_thread_prio - 1 < MAX_RT_PRIO) ? bustx_thread_prio:(MAX_RT_PRIO-1);
...
void aicwf_sdio_host_irq_enable(bool enable)
{
    struct aic_sdio_dev *sdiodev = g_rwnx_plat->sdiodev;
    if (enable) {
        sdio_claim_host(sdiodev->func);
        sdio_claim_irq(sdiodev->func, aicwf_sdio_hal_irqhandler);
        sdio_release_host(sdiodev->func);
...
void aicwf_sdio_irqrx(struct sdio_func *func)
{
    //printk("%s\n", __func__);
...
{
    //printk("%s\n", __func__);
    #ifndef CONFIG_IRQRX_THREAD
    aicwf_sdio_irqrx(func);
    #else
    struct aicwf_bus *bus_if = dev_get_drvdata(&func->dev);
    struct aic_sdio_dev *sdiodev = bus_if->bus_priv.sdio;
    aicwf_sdio_host_irq_enable(0);
    complete(&sdiodev->irqrx_trgg);
    #endif
}

#ifdef CONFIG_IRQRX_THREAD
int sdio_irqrx_thread(void *data)
{
    int ret = 0;
    struct aicwf_rx_priv *rx_priv = (struct aicwf_rx_priv *)data;
    struct aicwf_bus *bus_if = rx_priv->sdiodev->bus_if;
    ...
    /* bind to cpu0, to reduce scheduling latency */
    ...
    while (1) {
        if(kthread_should_stop()) {
            sdio_err("sdio irqrx thread stop\n");
            break;
        }

        /* if set completion sign, ret > 0; if timeout, ret == 0 */
        ret = wait_for_completion_interruptible_timeout(&sdiodev->irqrx_trgg, msecs_to_jiffies(2000));
        //printk("%s: ret=%d\n", __func__, ret);
        if (ret == 0) {
            printk("aicwf sdio polling\n");
        }
        if (ret >= 0) {
            aicwf_sdio_irqrx(sdiodev->func);
        }
    }
...
retry:
    ret = sdio_enable_func(sdiodev->func);
    if (ret < 0) {
        AICWFDBG(LOGERROR, "enable func fail %d %d.\n", retry_cnt, ret);
        if (retry_cnt < 3) {
            retry_cnt++;
            goto retry;
        }
...
void *aicwf_sdio_bus_init(struct aic_sdio_dev *sdiodev)
{
    ...
    if (IS_ERR(sdiodev->pwrctl_tsk)) {
        sdiodev->pwrctl_tsk = NULL;
        sdio_err("aicwf_sdio_pwrctl_thread create fail\n");
    }
    #endif

    #ifdef CONFIG_IRQRX_THREAD
    init_completion(&sdiodev->irqrx_trgg);
    sdiodev->irqrx_thread = kthread_run(sdio_irqrx_thread, (void *)bus_if->bus_priv.sdio->rx_priv, "aicwf_irqrx_thread");
    if (IS_ERR(sdiodev->irqrx_thread)) {
        sdiodev->irqrx_thread = NULL;
        sdio_err("sdio_irqrx_thread create fail\n");
    }
    #endif
