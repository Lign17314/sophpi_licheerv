...
void aicwf_usb_tx_flowctrl(struct net_device *ndev, bool state)
{
    ...
    printk("aicwf_usb_tx_flowctrl %d\r\n", state);
    if (state)
...
//printk("txcom:F:%d, P:%d\n", usb_dev->tx_free_count, usb_dev->tx_post_count);
    ...
    if (usb_dev->tx_free_count > AICWF_USB_TX_HIGH_WATER) {
        if (usb_dev->tbusy) {
            usb_dev->tbusy = false;
            #ifdef CONFIG_VNET_MODE
            aicwf_usb_tx_flowctrl(vnet_dev, false);
            #endif
        }
    }
...
static int aicwf_parse_usb(struct aic_usb_dev *usb_dev, struct usb_interface *interface)
{
    ...
    /* Check deviceclass */
    #ifndef CONFIG_RFTEST //CONFIG_USB_BT
    ...
    AICWFDBG(LOGINFO, "%s: NumInterfaces=%d\n", __func__, usb->actconfig->desc.bNumInterfaces);

    /* Check interface number */
    #ifdef CONFIG_RFTEST //CONFIG_USB_BT
    if (rftest_enable == 1) {
...
static int aicwf_usb_chipmatch(struct aic_usb_dev *usb_dev, u16_l vid, u16_l pid)
{
    if (pid == USB_PRODUCT_ID_AIC8801) {
        ...
        return 0;
    } else if (pid == USB_PRODUCT_ID_AIC8800DC || pid == USB_PRODUCT_ID_AIC8800DW) {
        usb_dev->chipid = PRODUCT_ID_AIC8800MC;
        AICWFDBG(LOGINFO, "%s USE AIC8800MC\r\n", __func__);
        return 0;
    } else if (pid == USB_PRODUCT_ID_AIC8800D41 || pid == USB_PRODUCT_ID_AIC8800D81) {
        usb_dev->chipid = PRODUCT_ID_AIC8800M40;
        AICWFDBG(LOGINFO, "%s USE AIC8800M40\r\n", __func__);
        return 0;
    } else if (pid == USB_PRODUCT_ID_AIC8800D81X2) {
        usb_dev->chipid = PRODUCT_ID_AIC8800M80X2;
        AICWFDBG(LOGINFO, "%s USE AIC8800M80X2\r\n", __func__);
        return 0;
    } else {
        return -1;
    }
...
static int aicwf_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    if(ret < 0) {
        usb_err("usb platform init fail\n");
        goto out_free_bus;
    }
    aicwf_hostif_ready();
    return 0;
...
extern void rwnx_aic_cdev_driver_deinit(void);
static void aicwf_usb_disconnect(struct usb_interface *intf)
{
    ...
    if (g_custom_msg_vnet.vnet_setting_cfm.keep_alive) {
        ...
        cancel_work_sync(&g_custom_msg_vnet.keep_alive.work_queue);
    }
    ...
    //if(timer_pending(&usb_dev->rwnx_hw->p2p_alive_timer)){
    //    del_timer(&usb_dev->rwnx_hw->p2p_alive_timer);
    //}
