...
#ifdef CONFIG_RFTEST
int rftest_enable = RFTEST_DISABEL;
module_param(rftest_enable, int, 0644);
MODULE_PARM_DESC(rftest_enable, "Enable rftest (0=off, 1=on)");
#endif
...
int custom_msg_cmd_parse(char *command, u32 cmd_len, u32 *argc, char buf[][APP_CMD_BUF_MAX])
{
    ...
    char *cmd_buf = NULL, *cmd_cpy = NULL, *token = NULL;
...
#ifdef CONFIG_RFTEST
int handle_rftest_cmd(struct net_device *net, char *command, u32 cmd_len)
{
    return aicwf_rftest_handler(command);
}
#endif
...
int handle_custom_msg(struct net_device *net, char *command, u32 cmd_len)
{
    int ret = 0;
    u32 argc = 0, data = 0;
    char (*buf)[APP_CMD_BUF_MAX] = NULL;
    aicwf_custom_msg_app_cmd cust_app_cmd;

    //printk("cmd: %s, %ld\n",command, strlen(command));
    buf = (char (*)[APP_CMD_BUF_MAX])kzalloc(APP_CMD_NUM_MAX*APP_CMD_BUF_MAX, GFP_KERNEL);
    if(!buf) {
    ...
    switch(ret) {
        case APP_CMD_CONNECT:
            ...
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_CONNECT_CFM;
        ...
        case APP_CMD_OPEN_BLE:
            ...
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_OPEN_BLE_CFM;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, command);
            break;
        case APP_CMD_CLOSE_BLE:
            ...
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_CLOSE_BLE_CFM;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, command);
            break;
        ...
        case APP_CMD_SCAN_WIFI:
            ...
            g_custom_msg_vnet.scan_pkt_recv_num = 0;
        ...
        case APP_CMD_HOST_OTA:
            ...
            if(argc != 2) {
                printk("Please input valid args: custom_msg vnet0 13 /your-filepath/update_app.bin\n");
                ret = -1;
                break;
            }
            ...
            host_ota_test(cust_app_cmd.file_path, OTA_TYPE_APP);
            ret = 0;
            break;
        case APP_CMD_HOST_FW_OTA:
            printk("APP_CMD_HOST_FW_OTA\n");
            if(argc != 2) {
                printk("Please input valid args: custom_msg vnet0 14 /your-filepath/update_fw.bin\n");
                ret = -1;
                break;
            }
            if(strlen(buf[1]) >= APP_CMD_BUF_MAX) {
            ...
            memcpy(cust_app_cmd.file_path, buf[1], strlen(buf[1]));
            cust_app_cmd.file_path[strlen(buf[1])] = '\0';
            host_ota_test(cust_app_cmd.file_path, OTA_TYPE_FW);
            ret = 0;
            break;
        case APP_CMD_HOST_SET_IP:
            ...
            break;
        case APP_CMD_START_P2PGO:
            printk("APP_CMD_START_P2PGO\n");
            if(argc != 4) {
                printk("Please input valid args: custom_msg vnet0 8 ssid password band\n");
                ret = -1;
                break;
            ...
            cust_app_cmd.ap_req.hdr.cmd_id = CUST_CMD_START_P2PGO_REQ;
            strncpy(cust_app_cmd.ap_req.ssid, buf[1], strlen(buf[1]));
            strncpy(cust_app_cmd.ap_req.pw, buf[2], strlen(buf[2]));
            cust_app_cmd.ap_req.ssid[strlen(buf[1])] = '\0';
            cust_app_cmd.ap_req.pw[strlen(buf[2])] = '\0';
            if(!strncasecmp(buf[3], "2.4G", 4))
                ...
                cust_app_cmd.ap_req.band = 0;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_START_P2PGO_IND;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.ap_req, sizeof(cust_app_cmd.ap_req), &cust_app_cmd.cmd_cfm, command);
            break;
        case APP_CMD_STOP_P2PGO:
            printk("APP_CMD_STOP_P2PGO\n");
            cust_app_cmd.common_req.cmd_id = CUST_CMD_STOP_P2PGO_REQ;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_STOP_P2PGO_IND;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, command);
            break;
        case APP_CMD_START_RFTEST:
            printk("APP_CMD_START_RFTEST\n");
            cust_app_cmd.common_req.cmd_id = CUST_CMD_START_RFTEST_REQ;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_START_RFTEST_CFM;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, command);
            break;
        case APP_CMD_START_HOSTBT:
            printk("APP_CMD_START_HOSTBT\n");
            cust_app_cmd.common_req.cmd_id = CUST_CMD_START_HOSTBT_REQ;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_START_HOSTBT_CFM;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, command);
            break;
        case APP_CMD_STOP_HOSTBT:
            printk("APP_CMD_STOP_HOSTBT\n");
            cust_app_cmd.common_req.cmd_id = CUST_CMD_STOP_HOSTBT_REQ;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = CUST_CMD_STOP_HOSTBT_CFM;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, command);
            break;
...
#ifdef CONFIG_RFTEST
int mcu_rf_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
    int ret = 0;
    char *command = NULL;
    int bytes_written = 0;
    ...
    bytes_written = handle_rftest_cmd(net, command, priv_cmd.used_len);
    if (bytes_written <= 0) {
        if (bytes_written == -1)
