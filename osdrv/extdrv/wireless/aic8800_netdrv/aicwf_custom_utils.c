...
int host_ota_test(const char * file_path, int ota_type)
{
    ...
    } else if (ota_type == OTA_TYPE_FW) {
        if (!g_custom_msg_vnet.vnet_setting_cfm.lzma_enable)
            ota_req->ota_step = OTA_STEP_FW_HEADER_WRITE;
        else
            ota_req->ota_step = OTA_STEP_FW_LZMA_CHECK;
    } else {
        printk("%s: type err %d\n", __func__, ota_type);
    }
