...
static uint8_t cmd_result[64];
static struct custom_msg_hostbt_channel_req hostbt_channel_req;
...
static int btsdio_tx_packet(struct btsdio_data *data, struct sk_buff *skb)
{
    ...
    cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
    cmd_cfm.cfm_id = CUST_CMD_HOSTBT_CHANNEL_CFM;
    ...
    hostbt_channel_req.hdr.cmd_id = CUST_CMD_HOSTBT_CHANNEL_REQ;
    hostbt_channel_req.data_len = skb->len;
    memcpy(hostbt_channel_req.bt_data, skb->data, skb->len);
    err = rwnx_tx_msg((u8 *)&hostbt_channel_req, sizeof(hostbt_channel_req), &cmd_cfm, cmd_result);

	if(err<0){
		printk("%s error %d",__func__,err);
		return err;
	}
