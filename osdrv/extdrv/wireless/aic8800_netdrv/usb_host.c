...
int aicwf_rwnx_usb_platform_init(struct aic_usb_dev *usbdev)
{
    ...
    #ifdef CONFIG_RFTEST
    rwnx_init_cmd_array();
    g_rwnx_hw = kzalloc(sizeof(struct rwnx_hw), GFP_KERNEL);
    if (!g_rwnx_hw) {
        printk("%s: g_rwnx_hw null\n", __func__);
        return -ENOMEM;
    }
    rwnx_cmd_mgr_init(&rwnx_plat->usbdev->cmd_mgr);
    g_rwnx_hw->usbdev = usbdev;
    g_rwnx_hw->cmd_mgr = &rwnx_plat->usbdev->cmd_mgr;
    rwnx_plat->usbdev->rwnx_hw = g_rwnx_hw;
    #endif

    ret = rwnx_platform_init(rwnx_plat, &drvdata);
    if (ret && rwnx_plat->deinit) {
